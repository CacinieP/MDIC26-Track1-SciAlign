...
MolAlign 是一个面向药物化学与分子科学领域的多模态对齐数据集，旨在让大语言模型准确理解分子实体的多维科学表征。当前版本包含 **120 条精选核心分子记录**，规划扩展至约 **1,200 条分子记录**，覆盖 26+ 药物类别，包括抗生素、抗癌药、神经递质、激素、维生素、天然产物等。扩展阶段通过 **MinerU Agent API** 批量解析真实科学论文（12 篇），获取 726,110 字符的优质结构化 Markdown 文本，为大规模文本描述生成提供真实数据基础。

数据集围绕每个分子实体构建**五类模态的跨模态对齐**：
...
| **MinerU Open Source** | 从论文 PDF 提取的图片、表格、文本（本地解析） | 5 篇论文 |
| **MinerU Agent API** | 通过 Agent API 远程解析论文 PDF，获取结构化 Markdown（免费，无需 Token） | 12 篇论文，726,110 字符 |
...
│     ├── MinerU Agent API 远程解析论文（12篇，726K字符）│
...
| 指标 | 当前版本 (v3.0) | 扩展目标 (v4.0) |
|------|----------------|----------------|
| 总记录数 | **120** | **~1,200** |
| 覆盖类别数 | **26** | **26+** |
| 2D 分子结构图 | **120** (100%) | **~1,200** (100%) |
| 3D 分子构象 (SDF) | **119** (99.2%) | **~1,190** (99%+) |
| 论文提取图片 | **12** (10%，来自13篇真实论文) | **~120** (10%+) |
| 实验生物活性数据 | **31** (25.8%，IC50/Ki/EC50/MIC) | **~310** (25%+) |
| PubChem 文本描述 | **113** (94.2%) | **~1,130** (94%+) |
| MinerU Agent API 论文解析 | **12 篇** | **~120 篇** |
| MinerU Agent API 解析文本量 | **726,110 字符** | **~7,000,000 字符** |
| 论文提取文本描述 | **13** (MinerU 解析) | **~130** |
| 中文文本描述 | **120** (100%) | **~1,200** (100%) |
| 验证通过率 | **120/120 (100%)** | **目标 99%+** |
| 构建失败 | **0** | — |
...
| **MinerU Agent API** | `POST https://mineru.net/api/v1/agent/parse/file` 远程解析（免费，无需 Token） | 批量远程解析 12 篇科学论文，获取结构化 Markdown 输出 |
...
**MinerU Agent API 实际使用证据**：

MinerU Agent API（`POST https://mineru.net/api/v1/agent/parse/file`）为免费远程解析服务，无需 API Token。在数据集扩展阶段，共提交 **12 篇真实科学论文**进行远程解析，**全部成功**完成，总计产出 **726,110 字符**的结构化 Markdown 文本。

| 论文文件名 | 对应分子 | Task ID | 解析文本量（字符） |
|-----------|----------|---------|-------------------|
| acetaminophen_pharmacology.pdf | Acetaminophen (MOL-000120) | `31984e49-0e05-...` | 76,039 |
| caffeine_molecules.pdf | Caffeine (MOL-000109) | `db1a6b1e-5820-...` | 49,453 |
| curcumin_pharmacology.pdf | Curcumin (MOL-000080) | `322f9eed-649b-...` | 51,141 |
| dopamine_neuroscience.pdf | Dopamine (MOL-000086) | `fd2f1298-04ea-...` | 78,931 |
| doxorubicin_anticancer.pdf | Doxorubicin (MOL-000023) | `868f2593-6da8-...` | 52,010 |
| fluoxetine_ijms.pdf | Fluoxetine (MOL-000038) | `658a83d5-76d6-...` | 66,064 |
| ibuprofen_pharmacology.pdf | Ibuprofen (MOL-000002) | `4cb5fba7-6ac0-...` | 77,588 |
| metformin_frontiers.pdf | Metformin (MOL-000033) | `b5eec853-2d47-...` | 67,162 |
| metformin_pharmaceutics.pdf | Metformin (MOL-000033) | `169780f7-869a-...` | 45,561 |
| morphine_opioid.pdf | Morphine (MOL-000058) | `1aed079e-5391-...` | 58,856 |
| paclitaxel_pharmacology.pdf | Paclitaxel (MOL-000021) | `cf999197-b775-...` | 46,500 |
| vitamins_pharmaceuticals.pdf | Vitamins (MOL-000100~104) | `a7d22358-f60b-...` | 56,805 |

> **合计：12 篇论文，12 篇成功解析，726,110 字符 Markdown 输出**
>
> 完整解析日志见 `data/raw/parsed_mineru_api/parsing_log.json`，解析输出存储于 `data/raw/parsed_mineru_api/`

**已知限制与解决方法**：

- **文件大小限制**：`omeprazole_pharmaceutics.pdf`（11 MB）超出 Agent API 10 MB 文件大小限制，未通过 Agent API 提交，改用 MinerU Open Source 本地解析
- **页数限制**：`fluoxetine_ijms.pdf` 和 `vitamins_pharmaceuticals.pdf` 初始提交时超出 20 页限制，通过指定 `page_range=1-15` 参数成功解决
...
#### (2) MinerU Agent API（远程解析，免费，无需 Token）

**用途**：通过 MinerU Agent API 远程解析科学论文 PDF，获取高质量结构化 Markdown 输出

**API 端点**：`POST https://mineru.net/api/v1/agent/parse/file`

**特点**：
- 免费，无需 API Token 即可使用
- 支持文件大小上限 10 MB
- 支持页数限制 20 页（可通过 `page_range` 参数指定范围）
- 返回结构化 Markdown + 图片 URL

**实际使用**：本数据集扩展阶段通过 MinerU Agent API 提交了 **12 篇真实科学论文**，**12 篇全部成功解析**，产出总计 **726,110 字符**的结构化 Markdown 文本。

**解析的论文**：

| 论文 | 对应分子 | 文本字符数 | Task ID（截取） |
|------|----------|-----------|----------------|
| acetaminophen_pharmacology.pdf | Acetaminophen | 76,039 | `31984e49-0e05-4531-82cd-b09e373b683f09` |
| caffeine_molecules.pdf | Caffeine | 49,453 | `db1a6b1e-5820-4650-ae1b-bab18a33f0a109` |
| curcumin_pharmacology.pdf | Curcumin | 51,141 | `322f9eed-649b-4503-9558-0a317edf9e8809` |
| dopamine_neuroscience.pdf | Dopamine | 78,931 | `fd2f1298-04ea-40bc-ba56-358fb4e55fff09` |
| doxorubicin_anticancer.pdf | Doxorubicin | 52,010 | `868f2593-6da8-4845-88b6-dd0fe24b881209` |
| fluoxetine_ijms.pdf | Fluoxetine | 66,064 | `658a83d5-76d6-4fbf-94f8-106d51067b1a09` |
| ibuprofen_pharmacology.pdf | Ibuprofen | 77,588 | `4cb5fba7-6ac0-44e1-80e9-4fe875ef263b09` |
| metformin_frontiers.pdf | Metformin | 67,162 | `b5eec853-2d47-4d82-9df3-3f3e5f20420d09` |
| metformin_pharmaceutics.pdf | Metformin | 45,561 | `169780f7-869a-43d3-9234-dac6d584fd9409` |
| morphine_opioid.pdf | Morphine | 58,856 | `1aed079e-5391-42ee-a918-5c92febe7d0d09` |
| paclitaxel_pharmacology.pdf | Paclitaxel | 46,500 | `cf999197-b775-45b4-939c-de1a1702367509` |
| vitamins_pharmaceuticals.pdf | Vitamins (5种) | 56,805 | `a7d22358-f60b-4861-8073-f271cc9318f209` |

**特殊情况处理**：

| 论文 | 问题 | 解决方案 |
|------|------|----------|
| fluoxetine_ijms.pdf | 超出 20 页限制 | 指定 `page_range=1-15`，成功解析 |
| vitamins_pharmaceuticals.pdf | 超出 20 页限制 | 指定 `page_range=1-15`，成功解析 |
| omeprazole_pharmaceutics.pdf | 文件大小 11 MB 超出 10 MB 限制 | 改用 MinerU Open Source 本地解析 |

**输出存储**：
- 解析日志：`data/raw/parsed_mineru_api/parsing_log.json`
- Markdown 输出：`data/raw/parsed_mineru_api/*.md`（12 个文件）
...
4. **论文知识提取验证**：通过 MinerU Agent API 解析的 12 篇论文（726,110 字符）已验证了从真实科学文献中提取分子相关知识的端到端流程，为扩展至 ~1,200 分子提供了可复用的 Pipeline。
...
| 质量指标 | 当前数值 (v3.0, 120条) | 扩展目标 (v4.0, ~1,200条) |
|----------|----------------------|--------------------------|
| SMILES 验证通过率 | 100% | 99%+ |
| Schema 验证通过率 | 100% | 99%+ |
| 2D 结构图覆盖率 | 100% (120/120) | 100% |
| 3D 构象覆盖率 | 99.2% (119/120) | 99%+ |
| 论文图片覆盖率 | 10% (12/120) | 10%+ |
| 实验数据覆盖率 | 25.8% (31/120) | 25%+ |
| MinerU Agent API 论文解析成功率 | 100% (12/12) | 95%+ |
| MinerU Agent API 解析文本总量 | 726,110 字符（12篇） | ~7,000,000 字符（~120篇） |
| MinerU 使用记录覆盖率 | 100% (120/120) | 100% |
| 实体关系覆盖率 | 100% (120/120) | 100% |
| 中文文本描述覆盖率 | 100% (120/120) | 100% |
| 英文文本描述覆盖率 | 94.2% (113/120) | 90%+ |
| 平均实体关系数/记录 | 1.88 | 1.5+ |
| 平均文本描述数/记录 | 2.17 | 2.0+ |
...
*报告版本：v3.1.0 | 数据集版本：v3.0.0（扩展至 v4.0 ~1,200 条规划中） | 最后更新：2026-05-31*
